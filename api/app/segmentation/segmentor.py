...
from ..models import ThrombusType
...
def cluster_image(
    ...
    kmeans = KMeans(
        n_clusters=k,
        init=(
            np.vstack((avg, *img[init_y, init_x]))
            if not fixed_center and seed is not None
            else "k-means++"
        ),
    ...
    def segment(
        ...
        if x2 and y2:
            clustered = cluster_image(
                window,
                K_CLOT if thrombus_type == ThrombusType.CLOT else K_FIBRIN,
            )
